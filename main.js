...
window.cursorMat = cursorMat; // Make accessible globally for UX syncing
const cursor = new THREE.Mesh(cursorGeo, cursorMat);
scene.add(cursor);
cursor.visible = false; // Initially hidden

// Sync UI and cursor color
function updateUX() {
    const nextTierSpan = document.getElementById('next-tier');
    if (nextTierSpan && window.cursorMat) {
        const tier = gameState.nextTier;
        nextTierSpan.innerText = `Tier ${tier}`;
        const colorHex = PALETTE[tier - 1] || 0xffffff;
        const colorStr = '#' + colorHex.toString(16).padStart(6, '0');
